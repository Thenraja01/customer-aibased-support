...
import { Plus, Edit2, Trash2, HelpCircle, AlertCircle, X, Check, ThumbsUp, ThumbsDown, Search } from "lucide-react";
...
export default function FAQManager({ orgId }: { orgId: string }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <HelpCircle size={18} className="text-muted-foreground" /> FAQ Management
          </h2>
          <p className="text-sm text-muted-foreground">Create and manage FAQs with approval workflow.</p>
        </div>
        <Button onClick={() => { setEditing(null); setShowForm(true); }}>
          <Plus size={16} className="mr-1" /> New FAQ
        </Button>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/50" onClick={resetForm}>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  rows={5}
                  className={`w-full rounded-lg border bg-card px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 resize-y min-h-[100px] ${

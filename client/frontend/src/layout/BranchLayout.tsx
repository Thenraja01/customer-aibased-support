...
import {
  LayoutDashboard, FileText, Settings, LogOut, Menu, Bell, X,
  Building2,
} from "lucide-react";
...
const navLinks = [
  { name: "Dashboard", path: "/branch/dashboard", icon: LayoutDashboard, permission: "ticket.assign" },
  { name: "Documents", path: "/branch/documents", icon: FileText, permission: "document.view_all" },
  { name: "Branches", path: "/branch/branches", icon: Building2, permission: "branch.view" },
];
...
export default function BranchLayout() {
  ...
  const { notifications, unreadCount, loadNotifications, loadUnreadCount, markAllRead } = useNotifications();
        ...
        <div className="flex items-center justify-between h-16 px-6 border-b dark:border-white/[0.06]">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Branch Admin</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 ml-auto">
            <div className="relative" ref={notifRef}>
              <button onClick={() => setShowNotifications(!showNotifications)} className="relative p-2 rounded-lg hover:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40" aria-label={`Notifications${unreadCount > 0 ? ` (${unreadCount} unread)` : ""}`}>
                <Bell size={18} />
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-destructive text-[9px] font-medium text-white flex items-center justify-center">

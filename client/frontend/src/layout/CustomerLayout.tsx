...
import {
  LayoutDashboard, MessageCircle, Ticket, HelpCircle,
  History, Bell, Settings, LogOut, Menu, X, ChevronDown, Plus
} from "lucide-react";
...
const navLinks = [
  ...
  { name: "Tickets", path: "/tickets", icon: Ticket, permission: "ticket.create" },
  { name: "FAQ", path: "/faq", icon: HelpCircle, permission: "knowledge.view" },
  { name: "Notifications", path: "/notifications", icon: Bell, permission: "notification.view" },
];
...
const chatGroup = {
  ...
  children: [
    { name: "New Chat", path: "/chat", icon: Plus, permission: "chat.view" },
    { name: "Chat History", path: "/chat-history", icon: History, permission: "chat.view" },
  ],
};
...
export default function CustomerLayout() {
  ...
  const { notifications, unreadCount, loadNotifications, loadUnreadCount, markRead, markAllRead, loadNotifications: refreshNotifications } = useNotifications();

  const brandName = orgSettings?.chatbot_name || user?.organization_id?.name || "SupportAI";
  ...
  const visibleChatChildren = chatGroup.children.filter((child) => !child.permission || can(child.permission));

  // Auto-expand chat group when on a chat sub-route
  const isOnChatRoute = visibleChatChildren.some(
    (c) => location.pathname === c.path || location.pathname.startsWith(c.path + "/")
  );

  useEffect(() => {
    if (isOnChatRoute) setChatOpen(true);
  }, [isOnChatRoute]);
  ...
  useEffect(() => {
    ...
    loadNotifications();
    loadUnreadCount();
  }, []); // Empty deps to prevent reloads on every route change
  ...
  // Stable handler for marking as read with refresh
  const handleMarkRead = useCallback(async (notifId: string) => {
    await markRead(notifId);
    await loadUnreadCount();
    await refreshNotifications();
  }, [markRead, loadUnreadCount, refreshNotifications]);

  // Stable handler for marking all as read
  const handleMarkAllRead = useCallback(async () => {
    await markAllRead();
    await loadUnreadCount();
    await refreshNotifications();
  }, [markAllRead, loadUnreadCount, refreshNotifications]);
      ...
      >
        <div className="flex items-center justify-between h-16 px-6 border-b dark:border-white/[0.06]">
          <div className="flex items-center gap-3 min-w-0">
            {brandLogo ? (
              <img src={brandLogo} alt={brandName} className="h-7 w-auto shrink-0" />
            ) : (
          ...
          <button 
            type="button"
            className="lg:hidden p-1.5 rounded-lg hover:bg-muted text-muted-foreground shrink-0" 
            onClick={() => setSidebarOpen(false)} 
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          {/* Flat nav links before Chat group */}
          ...
          <div>
            ...
            <div
              className={cn(
                ...
                chatOpen ? "max-h-40 opacity-100" : "max-h-0 opacity-0"
              )}
            >
              <div className="ml-4 mt-1 space-y-0.5 border-l-2 border-primary/20 pl-3">
                {visibleChatChildren.map((child) => {
          ...
          {/* Remaining flat nav links after Chat group */}
          {navLinks.slice(1).filter(link => link.name !== "Chat").map((link) => {
            const Icon = link.icon;
            const isActive = location.pathname === link.path || location.pathname.startsWith(link.path + "/");
        ...
        <div className="p-4 border-t dark:border-white/[0.06] space-y-1">
          <Link to="/profile" 
            className="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-muted-foreground rounded-lg hover:bg-muted dark:hover:bg-white/[0.04] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
          >
          ...
          <div className="flex items-center gap-2 ml-auto">
            <div className="relative" ref={notifRef}>
              <button 
                ...
                className="relative p-2 rounded-lg hover:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40" 

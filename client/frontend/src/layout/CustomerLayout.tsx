      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          {navLinks.map((link) => {
            ...
            const isActive = location.pathname === "/chat" || location.pathname === "/chat-history" || location.pathname.startsWith("/chat-history/");

            if (isChatLink) {
              return (
                <div key="chat-dropdown" className="space-y-1">
                  <div
                    onClick={() => setChatDropdownOpen(!chatDropdownOpen)}
                    className={cn(
                      "flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40",
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Icon size={18} /><span>Chat</span>
                    </div>
                    <ChevronDown size={16} className={cn("transition-transform duration-200", chatDropdownOpen ? "rotate-180" : "")} />
                  </div>

                  {chatDropdownOpen && (
                    <div className="pl-8 pr-2 space-y-1 pt-1 pb-2">
                      <button
                        onClick={() => { navigate("/chat"); setChatDropdownOpen(false); }}
                        className="flex items-center gap-2.5 w-full px-3 py-2 rounded-md text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
                      >
                        <Plus size={14} /><span>New Chat</span>
                      </button>
                      <button
                        onClick={() => { navigate("/chat-history"); setChatDropdownOpen(false); }}
                        className="flex items-center gap-2.5 w-full px-3 py-2 rounded-md text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
                      >
            ...
            if (link.path === "/chat-history") {
              return null; // Handled inside Chat dropdown
            }

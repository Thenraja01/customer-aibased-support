        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <button
              ...
              className="group flex items-center gap-4 rounded-xl border bg-card dark:bg-card/50 dark:border-white/[0.06] p-6 shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 text-left"
            >
              ...
              <ArrowRight size={15} className="text-muted-foreground group-hover:text-foreground group-hover:translate-x-0.5 transition-all shrink-0" />
            </button>
          ...
          {chats.length > 0 && (
            <div className="rounded-xl border bg-card dark:bg-card/50 dark:border-white/[0.06] shadow-xs">
              <div className="px-4 py-3 border-b dark:border-white/[0.06] flex items-center justify-between">
                ...
                <button onClick={() => navigate("/chat-history")} className="text-xs text-primary hover:underline">View all</button>
              </div>
              <div className="divide-y dark:divide-white/[0.04]">
                {chats.slice(0, 5).map((chat: any) => (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => { if (e.key === "Enter") navigate("/chat", { state: { chatId: chat._id } }); }}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5">{new Date(chat.created_at).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-md ml-3 shrink-0 ${chat.status === "open" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-muted text-muted-foreground"}`}>
          ...
          {tickets.length > 0 && (
            <div className="rounded-xl border bg-card dark:bg-card/50 dark:border-white/[0.06] shadow-xs">
              <div className="px-4 py-3 border-b dark:border-white/[0.06] flex items-center justify-between">
                ...
                <button onClick={() => navigate("/tickets")} className="text-xs text-primary hover:underline">View all</button>
              </div>
              <div className="divide-y dark:divide-white/[0.04]">
                {tickets.slice(0, 5).map((ticket: any) => {
                  const statusColor = ticket.status === "open" ? "bg-primary/10 text-primary" : ticket.status === "in_progress" ? "bg-accent text-accent-foreground" : ticket.status === "resolved" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-muted text-muted-foreground";
                  return (
                    <div key={ticket._id} className="px-4 py-3 flex items-center justify-between hover:bg-muted/50 dark:hover:bg-white/[0.03] transition-colors">
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5">{new Date(ticket.created_at).toLocaleDateString()}</p>
                      </div>
                      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-md ml-3 shrink-0 ${statusColor}`}>{ticket.status?.replace("_", " ")}</span>
                    </div>
                  );
                })}
              </div>

...
import { Send, Globe, Building2, AlertTriangle, Info, CheckCircle, AlertCircle } from "lucide-react";
import { useToast } from "@/components/ui/toast";
import { NotificationAPI } from "@/api/notification.api.js";
import { MockAdminAPI as AdminAPI } from "@/api/mockAdminApi.js";
...
export default function SendNotificationPage() {
  ...
  useEffect(() => {
    AdminAPI.getOrganizations({ limit: 200 })
      .then((res: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (scope === "org" && !selectedOrg) {
      toast.error("Validation", "Please select an organization");
      return;
    }
    ...
    try {
      if (scope === "all") {
        const res = await NotificationAPI.broadcastToAll({ title, message, type, link: "" });
        toast.success("Sent", `Notification sent to ${res.data.count || "all"} users across all organizations`);
      } else {
        const res = await NotificationAPI.broadcastToOrgById(selectedOrg, { title, message, type, link: "" });
        toast.success("Sent", `Notification sent to ${res.data.count || "all"} users in the selected organization`);
      }
      setTitle("");
      setMessage("");
      setType("info");
    ...
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Broadcast a notification to all users or a specific organization
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-xl border bg-card p-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all border ${
                scope === "all"
                  ? "ring-2 ring-primary/40 border-primary shadow-sm bg-primary/5"
        ...
        {scope === "org" && (
          <div>
            <label htmlFor="org" className="block text-sm font-medium mb-1.5">Organization</label>
            <select
              id="org"
              value={selectedOrg}
        ...
        <div className="flex items-center gap-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300 px-4 py-3 text-sm">
          ...
          <span>
            {scope === "all"
              ? "This will send a notification to every active user across all organizations."
              : "This will send a notification to every active user in the selected organization."}
          </span>
        </div>

        <button
          type="submit"
          disabled={sending || !title.trim() || !message.trim() || (scope === "org" && !selectedOrg)}

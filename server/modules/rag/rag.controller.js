...
export const ingest = async (req, res) => {
  try {
    ...
    const chunks = await ragService.ingestDocument(documentId, organizationId, text);
...
export const query = async (req, res) => {
  try {
    ...
    const organizationId = req.user?.organization_id?._id || req.user?.organizationId;
    const results = await ragService.hybridQuery(q, organizationId, documentId, 5, userId, chatId);
